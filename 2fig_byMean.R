...
gedi_gridded <- read.csv("gedi_calval_shot_filtered_gridded.csv")
gedi_gridded <- read.csv("gedi_calval_shot_filtered_gridded_optimal.csv")
...
#is1 <- raster('IS1_raster_14km_mean_growingSeason.tif')
...
tiff("plot_filtered_gridded_mean_optimal_filterDis_is1.tiff", width = 6, height = 5, units = "in", res = 300)
p
dev.off()
...
p2
# Save the plot as a TIFF file
tiff("plot_median_mean_filtered_mean_optimal_filterDisturb_is1.tiff", width = 6, height = 5, units = "in", res = 300)
p2
dev.off()